...
class Compatibility {
  ...
  doFix () {
    ...
    if (audioChanged) {
      ...
      if (!this._isSegmentsContinuous() && ops && ops.start !== undefined) {
        ...
        const sam0 = this.audioTrack.samples[0]
  ...
  doFixAudio (first, streamChangeStart) {
    ...
    const silentFrame = AAC.getSilentFrame(meta.originCodec, meta.channelCount)
    ...
    for (let i = 1; i < audioSamples.length;) {
      ...
      if (delta >= 10 * iRefSampleDuration) {
        ...
        logger.warn(this.TAG, `inject ${missingCount} audio frame for ${delta} ms gap, index=${i}`)
