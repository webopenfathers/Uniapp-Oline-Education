...
var caculate = _xgplayerHelperUtils.common.caculate;
...
var Compatibility = function () {
  ...
  _createClass(Compatibility, [{
    ...
    value: function doFix(format) {
      ...
      if (this.videoTrack.samples.length) {
        ...
        if (_xgplayerHelperUtils.logger.long) {
          _xgplayerHelperUtils.logger.log(this.TAG, this.videoTrack.samples.slice());
        }
      }
      ...
      if (this.audioTrack.samples.length) {
        ...
        if (_xgplayerHelperUtils.logger.long) {
          _xgplayerHelperUtils.logger.log(this.TAG, this.audioTrack.samples.slice());
        }
      }
      ...
      _xgplayerHelperUtils.logger.log(this.TAG, 'lastSeg:' + this._lastSegmentId + ' , currentSeg:' + this._currentSegmentId + ', discontinue:' + (ops && ops.discontinue) + ' vSamp0:' + (vSamp0 && vSamp0.dts) + ' aSamp0:' + (aSamp0 && aSamp0.dts));
      ...
      if (videoChanged) {
        ...
        if (!this._isSegmentsContinuous() && ops && ops.start !== undefined) {
          _xgplayerHelperUtils.logger.log(this.TAG, 'fix video for _isSegmentsContinuous()');
          this.videoLastSample = null;
      ...
      if (audioChanged) {
        ...
        if (!this._isSegmentsContinuous() && ops && ops.start !== undefined) {
          _xgplayerHelperUtils.logger.log(this.TAG, 'fix audio for _isSegmentsContinuous()');
    ...
    value: function doFixVideo(first, streamChangeStart) {
      ...
      _xgplayerHelperUtils.logger.log(this.TAG, 'doFixVideo:: lastVideoDts: ' + this.lastVideoDts + ' ,  _videoLargeGap: ' + this._videoLargeGap + ' ,streamChangeStart:' + streamChangeStart + ', lastVideoSample:[dts=' + (this.videoLastSample && this.videoLastSample.dts) + ' , pts=' + (this.videoLastSample && this.videoLastSample.pts) + '] ,  firstDTS:' + firstSample.dts + ' ,firstPTS:' + firstSample.pts + ' ,lastDTS:' + videoSamples[len - 1].dts + ' , lastPTS: ' + videoSamples[len - 1].pts);
    ...
    value: function doFixAudio(first, streamChangeStart) {
      ...
      var silentFrame = _aac2.default.getSilentFrame(meta.codec, meta.channelCount);
      ...
      for (var _i7 = 1; _i7 < audioSamples.length;) {
        ...
        if (delta <= -1 * iRefSampleDuration) {
          _xgplayerHelperUtils.logger.warn('drop 1 audio sample for ' + delta + ' ms overlap');
        ...
        if (delta >= 10 * iRefSampleDuration) {
          ...
          _xgplayerHelperUtils.logger.warn(this.TAG, 'inject ' + missingCount + ' audio frame for ' + delta + ' ms gap');
      ...
      audioSamples.forEach(function (sample, idx) {
        ...
        _this2.audioUnsyncTime = caculate.fixedFloat(_this2.audioUnsyncTime + unSyncDuration, 2);
    ...
    value: function fixChangeStreamVideo(changeIdx) {
      _xgplayerHelperUtils.logger.log(this.TAG, 'fixChangeStreamVideo(), changeIdx=', changeIdx);
    ...
    value: function fixChangeStreamAudio(changeIdx) {
      _xgplayerHelperUtils.logger.log(this.TAG, 'fixChangeStreamAudio(), changeIdx=', changeIdx);
    ...
    value: function detectVideoLargeGap(isTs, nextDts, firstSampleDts) {
      if (nextDts === null) {
        return;
      }
      var delta = isTs ? 1000 : 10000;
    ...
    key: 'doFixLargeGap',
    value: function doFixLargeGap(samples, gap, isTs) {
